...
| 2. 选择并调用不同模型服务（本站待收录） | 隔离服务选择和认证差异 | 使用同一个入口切换不同 Provider |
| 3. 接入阿里云百炼（本站待收录） | 用现有协议适配器组装百炼 Provider | 完成一次真实百炼模型请求 |
...
- 已收录：本篇总览、[第 1 章：定义一次模型调用](../01-unified-contract/)
- 第 2～10 章尚未收录，后续按章节顺序更新。
